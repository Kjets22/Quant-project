"""extract_v4_trade.py — find a CLEAN NVDA v4 (15-min/4:1) trade that really ran to target."""
import json
...
d = df.set_index("timestamp").resample("15min").agg(
...
win = (ts >= np.datetime64("2026-06-08")) & (ts < np.datetime64("2026-06-27"))
best = None
for i in np.where(win)[0]:
    a = A[i]
    if not np.isfinite(a) or a <= 0.3:
        continue
    tgt, stop = c[i] + TP * a, c[i] - SL * a
    j = i + 1
    hit = None
    while j < min(i + 25, len(c)):
        if l[j] <= stop:
            hit = "STOP"; break
        if h[j] >= tgt:
            hit = "TARGET"; break
        j += 1
    if (hit == "TARGET" and 4 <= (j - i) <= 16
            and pd.Timestamp(ts[j]).date() == pd.Timestamp(ts[i]).date()):   # same-day, no gap
        best = (i, j, a)
        break
...
out = {"candles": cs, "entry_idx": int(i - lo), "exit_idx": int(xj - lo),
       ...
       "entry_ts": str(pd.Timestamp(ts[i])), "exit_ts": str(pd.Timestamp(ts[xj]))}
Path("runs").mkdir(exist_ok=True)
Path("runs/v4_trade.json").write_text(json.dumps(out))
