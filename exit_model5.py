"""
exit_model5.py — FIXED accounting (per user's correct catch). The early exit may only fire
STRICTLY BEFORE the bar that resolves the trade, so it always exits ABOVE the -1 ATR stop
(never at/below it). Stop loss stays on as the backstop. This removes the idealized-vs-real
fill mismatch, so cutting a stop-bound trade can only SAVE (or do nothing) -- never lose.
Reports the honest result. Standalone; touches no frozen snapshot.
"""
...
def main():
    ...
    for h, l, c, A, Xnp, tests in store:
        for i, res, ex in tests:
            ...
            rows, cjs = [], []
            for j in range(i + 1, ex):                 # STRICTLY before the resolving bar ex
                if np.isfinite(Xnp[j]).all():
    ...
    print(f"=== EXIT MODEL v5 (FIXED: exit only BEFORE the resolving bar) on v4 [DEV] — {len(pre)} trades ===")
    print(f"  baseline (hold, stop intact) total = {base.sum()*100:+.0f}%\n")
    print(f"  {'P(tgt)<':>8} {'#cut':>6} {'stop-bound cut':>14} {'recover/win cut':>15} {'integrated total%':>17}")
    for THR in THRS:
        rets, sc, rc = [], 0, 0
        for b, ci, cjs, ptgt, res in pre:
            ...
            rets.append(er)
        r = np.array(rets)
        print(f"  {THR:>8} {sc+rc:>6} {sc:>14} {rc:>15} {r.sum()*100:>+16.0f}%")
    print("\n  Now cutting a stop-bound trade can only help (exit is above the stop). If integrated")
    print("  is >= baseline, your logic holds and my earlier -51% was the accounting artifact.")
