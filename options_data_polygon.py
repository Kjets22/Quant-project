...
def spy_daily(start, end):
    s, k = _session_key()
    j = _get(s, k, f"https://api.polygon.io/v2/aggs/ticker/SPY/range/1/day/{start}/{end}",
...
def list_contracts(exp_gte, exp_lte, strike_lo, strike_hi):
    """All expired SPY contracts (both types) in the expiry + strike window."""
    s, k = _session_key()
    out = []
    for ctype in ("call", "put"):
        url = "https://api.polygon.io/v3/reference/options/contracts"
        params = {"underlying_ticker": "SPY", "contract_type": ctype,
...
def build_chain(start: str, end: str, fridays_only: bool = True,
                band: float = 0.10) -> pd.DataFrame:
    """Assemble the per-(date, contract) chain DataFrame for [start, end]."""
    spy = spy_daily(start, end)
    spy["date"] = pd.to_datetime(spy["date"])
    spot = {d.date(): c for d, c in zip(spy["date"], spy["close"])}
    smin, smax = spy["close"].min(), spy["close"].max()

    contracts = list_contracts(start, end, smin * (1 - band), smax * (1 + band))
    if fridays_only:
    ...
    for i, c in enumerate(contracts):
        ...
        for b in contract_aggs(tick, start, end):
            ...
            rows.append({
                "date": d, "underlying": "SPY", "expiry": exp, "strike": K,
...
if __name__ == "__main__":
    ...
    end = sys.argv[2] if len(sys.argv) > 2 else "2025-06-20"
